...
export default function ExampleLazyPage() {
  const { t } = useI18n();
  ...
  const prepareExample = async () => {
    if (selectedClient) {
      setPageTitle((await getConfiguration('example.title', 'example page', token))+' - Lazy Mode');
    }
  }
    ...
    <div>
      ...
      >
        <div className="text-sm text-gray-500">
          <br />Source code: <code>modules/Example/frontend/ExampleLazyPage.tsx</code>
        </div>
      </UniversalPage>
    </div>
  );
